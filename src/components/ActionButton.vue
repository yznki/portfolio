<script setup lang="ts">
  ...
  const buttonStyles = cva(
    "inline-flex gap-2 items-center justify-center rounded-lg px-4 py-2 ease-in-out transition-colors duration-500",
    {
      variants: {
        intent: {
          ...
          tertiary: "border border-primary-500 text-primary-500 hover:bg-primary-500 hover:bg-opacity-20 active:bg-primary-700",
        },
        disabled: {
...
<template>
  <span>
    <button :class="buttonStyles({ intent: intent, disabled: disabled, fullWidth: fullWidth })" @click="onClick" :disabled="disabled">
      <span v-if="icon">
